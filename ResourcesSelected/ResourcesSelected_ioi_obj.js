...
(function( $ ){
	...
	// ioi: resources table id
	    ...
	    this.initial_resources = {node:'none', timeslot:0}; // ioi
            this.initial_resources.node = sliver_urns[0]; // ioi: We make a copy of the object



            if (this.current_resources == null) {
                //this.current_resources = sliver_urns;
		this.current_resources = {node:'none', timeslot:0}; // ioi
		this.current_resources.node = sliver_urns[0]; // ioi

                /* We simply add to the ResourceSelected table */
                var newlines=Array();
                $.each(sliver_urns, function(index, urn) {
		    var SPAN = "<span class='ui-icon ui-icon-close ResourceSelectedClose' id='"+urn+"'/>"; // ioi
		    var slot = "<span id='resource_"+urn+"'>0</span>"; // ioi
                    //newlines.push(Array('attached', urn, " ", " ")); // ioi: added last element
		    newlines.push('attached', urn, slot, SPAN); // ioi: added last element
	    ...
	    var resource_time = "arbitrary_ioi"; // ioi : temp value, retrieve the real one from the event

            my_oTable.fnClearTable();
            $.each(resources, function(i, r) { 
console.log(r);
console.log(initial);
		// TODO:Now initial is not an array but an object initial.node
                //var type = ($.inArray(r, initial) == -1) ? 'add' : 'attached';
		var type = (r !== initial.node) ? 'add' : 'attached';
		//var time = ($.inArray(r, initial) == -1) ? resource_time : 'none'; //ioi
		var node = "none";
